...
from utils import get_tensors, ref_attn
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, bias=None, causal=False, softmax_scale=None):
        o = flash_attn_triton(q, k, v, causal=causal, sm_scale=softmax_scale).half()
...
def triton_flash_func(q, k, v, bias=None, causal=False, softmax_scale=None):
...
def causal_test(BS, HEAD, SEQLEN, DIM, causal):
    ...
    sm_scale = 0.5
    # TODO: 为什么开启多轮测试,后 triton实现比torch速度慢?
    epoch = 100
    ...
    print(f"causal={causal} value check passed")

def test_attention():
    BS, HEAD, SEQLEN, DIM = 1, 2, 10240, 64
    causal_test(BS, HEAD, SEQLEN, DIM, causal=False)
    causal_test(BS, HEAD, SEQLEN, DIM, causal=True)

def test_speed():
    BS, HEAD, SEQLEN, DIM = 1, 128, 10240, 64
    causal=True
    torch.manual_seed(20)
    dtype = torch.float16
    q, k, v = get_tensors(BS, HEAD, SEQLEN, DIM)
    ...
    fn_tri = lambda: triton_flash_func(q, k, v, causal=causal, softmax_scale=sm_scale)
    ms_tri = triton.testing.do_bench(fn_tri, warmup=warmup, rep=epoch)

    for _ in range(warmup):
        flash_attn_func_cuda(q, k, v, softmax_scale=sm_scale, causal=causal)

    torch.cuda.synchronize()
    time_cuda = time.time()
    flash_attn_func_cuda(q, k, v, softmax_scale=sm_scale, causal=causal)
    torch.cuda.synchronize()
    time_cuda = time.time() - time_cuda
    print(f"\nwarm_up={warmup}", "causal = {} tri time: {:.4f}ms, cuda time: {:.4f}ms".format(causal, ms_tri, time_cuda * 1000))
